...
_ds_lease_read() {
  ...
  if ! command -v jq >/dev/null 2>&1; then
    printf '⚠ dev-server-lease: jq not installed; cannot read lease at %s\n' "${f}" >&2
    return 1
  fi
...
_ds_lease_port_pid() {
  if command -v ss >/dev/null 2>&1; then
    ...
    lsof -ti :"${DS_LEASE_PORT}" -sTCP:LISTEN 2>/dev/null | head -1
  else
    # No tool available; can't determine.
    return 1
  fi
}
...
ds_lease_acquire() {
  ...
  if _ds_lease_read; then
    if [ "${DS_LEASE_OWNER_PID}" = "${pid}" ]; then
      # Already the owner; refresh timestamp+sha to track progress.
    ...
    if _ds_lease_pid_alive "${DS_LEASE_OWNER_PID}" \
      ...
      # Live owner; do not overwrite.
      return 7
    fi
    # Stale lease; clear + acquire.
...
ds_lease_inspect() {
  ...
  if [ "${have_lease}" -eq 1 ] && _ds_lease_pid_alive "${DS_LEASE_OWNER_PID}"; then
    lease_pid_alive=1
  fi

#!/usr/bin/env bash
# M2a post-hoc probe #2 — pi-e122-r5 over the Responses route.
# Mandatory work item (1) from report.md §Verdict: re-probe the E122 rung-5
# cell through Pi's `api: "openai-responses"` before committing the
# supervision-tier design. One attempt, flagged post-hoc per rubric.
#
# Derived from run-cell.sh's pi-e122-r5 case; the ONLY treatment changes:
#   - provider `openrouter-responses` (pi-openrouter-responses.js extension)
#     instead of Pi's builtin `openrouter` (chat-completions catalog entry)
#   - cell name/worktree suffixed `-responses`
# Held constant: frozen rung-5 brief text, job/report contract, Pi TAIL,
# --thinking medium, 30-min cap, -ne -ns --offline, worker base b82b404d.
set -u
ATTEMPT="${1:-1}"
SPIKE_DIR="$(cd "$(dirname "$0")" && pwd)"
BRIEF="${SPIKE_DIR}/../2026-07-16-difficulty-ladder/briefs/rung-5.md"
WT="/home/administrator/Code/tuxlink/worktrees/bd-tuxlink-7raoe-m2a-pi-e122-r5-responses"
...
TRANSCRIPT="${SDD}/rung-${RUNG}-pi-responses-transcript${SUFFIX}.txt"
...
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) cell-pi-e122-r5-responses attempt-${ATTEMPT} dispatched" >> "${SDD}/timing.log"
...
RC=${PIPESTATUS[0]}
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) cell-pi-e122-r5-responses attempt-${ATTEMPT} finished (exit ${RC})" >> "${SDD}/timing.log"
exit "$RC"

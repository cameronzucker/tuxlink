...
- Working tree **clean** (all committed + pushed). Branch is up to date with `origin/bd-tuxlink-vfb3/cms-password-change`.
- **CI:** the final commit (`eefa5fec`, shipped-path contract fix) was pushed; CI (`verify` = cargo test, `build-linux`) was **pending** at handoff time. **Next session: confirm CI is green** (`gh pr checks 787`) before building further Rust — the contract fix was written without a local cargo compile (this Pi can't cold-build), so CI is its first compile.
- This handoff is committed on the **feature branch** (not the operator's main branch) because main is lease-locked; move/relocate per your normal flow if desired.
...
- **0 · Backend account-API command layer** — spec'd ([v2 spec](../../docs/superpowers/specs/2026-06-17-cms-account-api-command-layer-design.md)). Shipped path corrected; **remaining: build `account_create` / `account_exists` / `account_validate_password` / `account_set_recovery_email` / `account_send_recovery` / `account_remove`** on a shared `post_account_form` helper + `AccountApiError` (generalize `PasswordChangeError`) + **per-base-callsign mutex** on mutating ops + typed read outcomes + **tactical-input rejection** + `account_remove` `UnknownOutcome`/`account_exists` reconciliation. Register in `src-tauri/src/lib.rs`. TDD offline (no key needed); operator runs the live lifecycle later. **A Codex round on the corrected encoding before "done."**

...
**Branch:** `bd-tuxlink-wpqwy/elmer-model-access-onboarding` · **PR:** #984 (ready) · **Head:** `e5f05fd3`
...
1. **CI green by headSha `e5f05fd3`** — verify the `verify` job (typecheck/vitest/clippy/cargo test, both arches) is green BEFORE merge. This is the ONLY gate that has compiled the Rust (T3/4/5). The earlier head failed on the AppShell test (now fixed) — confirm this run is fully green.

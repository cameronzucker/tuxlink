...
- **Persistence with a safety net:** popped/docked choice + window geometry survive restart; a missing monitor at launch is safe by construction — the window-state plugin (X11) and the compositor (Wayland) place the window on a connected monitor, so the surface restores popped and reachable, never stranded off-screen. *(Amended 2026-07-15: the original "falls back to docked" mechanism is unimplementable on both display backends — the plugin pre-empts the off-screen state on X11, and Wayland clients cannot query window position. The preserved invariant is "never stranded, never unreachable." See the dockable-surfaces design spec §3 adversarial findings.)*

...
- **↗ Pop out** in the surface header moves the surface to its own OS window; **⇤ Dock back** in that window's title bar (or closing it) returns it inline. The main window's menu item reads "Routines ↗" while popped and focuses the window instead of swapping the pane. *AMENDMENT 2026-07-15 (AMD-2): ⇤ and ✕ both return the surface inline, with different presentation — ⇤ foregrounds the surface at its inline placement; ✕ returns it to inline availability (pathway affordances revert) without commandeering the main pane, reading pane, or active dock tab. The invariant preserved: the surface is inline-available again, nothing vanishes. The un-split reading made ✕ evict the operator's inbox — the exact thing the feature exists to protect. See the dockable-surfaces design spec §5.*
...
- **Persistence with a safety net:** popped/docked choice + window geometry survive restart; a missing monitor at launch is safe by construction — the window-state plugin (X11) and the compositor (Wayland) place the window on a connected monitor, so the surface restores popped and reachable, never stranded off-screen. *AMENDMENT 2026-07-15 (AMD-1): the original "falls back to docked" mechanism is unimplementable on both display backends — the plugin pre-empts the off-screen state on X11, and Wayland clients cannot query window position. The preserved invariant is "never stranded, never unreachable"; the guarantee is launch-time, with main-side dock-back actions covering mid-session monitor loss. See the dockable-surfaces design spec §3 adversarial findings.*

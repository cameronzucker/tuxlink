...
**Status:** Revised post 5-round adrev (commit `26a0ffb` → `68a698c`; 50 findings; 8 P0 + 15 P1 + 11 P2 applied). Further amended post 4-round plan-review (commit `68a698c` → this commit; 38 plan-review findings + the `project_fork_enables_aggressive_deletion` insight: Pat's web UI is now DELETED rather than modified, eliminating npm/webpack/Docker entirely from tuxlink-pat's build chain). See §7 for full adrev disposition; the plan-review revision is captured in the plan's §8 disposition.
...
4. **Delete Pat's web UI entirely from tuxlink-pat** (`rm -rf web/`; remove `pat/web` import from `api/api.go`; remove web routes from Pat's `http` subcommand). Per the [`project_fork_enables_aggressive_deletion`](../../../../.claude/projects/-home-administrator-Code-tuxlink/memory/project_fork_enables_aggressive_deletion.md) memory: Pat's web UI is **redundant** in tuxlink's architecture (tuxlink wraps Pat in Tauri; users see Tauri UI, never Pat's web UI). Per spec §4.4, tuxlink-pat is positioned as tuxlink's engine, NOT a standalone Pat replacement. Deleting the web UI eliminates: ~6000 LoC of frontend code, the entire webpack/npm/Docker build chain, the npm supply-chain risk surface (Shai-Hulud/QIX/axios class), the R3+R4 web-UI-cred-cascade findings, and the create-account-modal residual flow. Cred-refactor patch becomes pure-Go engine refactor with much cleaner scope. (Replaces the earlier "modify web/src/* + web/dist/*" approach which would have inherited Pat's npm dependency tree.)
...
│                                                                       │
│  web/ (entire directory): DELETE per fork-enables-aggressive-deletion │
│      rm -rf web/ — Pat's web UI redundant in tuxlink's Tauri-wrapped  │
│      architecture; tuxlink users never see Pat's web UI               │
│      api/api.go: remove pat/web import + route registrations          │
│      Eliminates webpack/npm/Docker chain entirely (~6000 LoC delete)  │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
```
...
| **`web/` (entire directory)** | **This patch (DELETE) — REPLACES the prior 3 web-MODIFY rows** | `rm -rf web/`. Eliminates: `web/Dockerfile`, `web/make.bash`, `web/package.json`, `web/package-lock.json`, `web/webpack.config.js`, `web/src/*` (config.html, index.html, js/, scss/, static/, template.html), `web/dist/*` (webpack-bundled output), `web/web.go` (the go:embed handler + UI/Dist handlers), `web/web_test.go`. ~6000 LoC of frontend deleted. Eliminates npm/webpack/Docker from tuxlink-pat's build chain entirely. Per ADR 0011 §1 + spec §4.4 ("tuxlink-pat is tuxlink's engine; tuxlink users see Tauri UI, never Pat's web UI"). |
| **`api/api.go`** | **This patch (MODIFY) — extended from earlier row** | (Already in this row: delete `RedactedPassword` machinery.) ALSO: remove `"github.com/la5nta/pat/web"` import (line 26); remove all route registrations / HTTP-handler wiring that consumes `web.DistHandler()` / `web.UIHandler()`. The `pat http` subcommand continues to serve the JSON API but no longer serves any browser UI. |
...
- **Upstream re-introduces `web/` directory at merge time:** since this patch DELETES Pat's web UI entirely, every future upstream-merge cycle (per ADR 0011 §5 opportunistic sync) will see `web/` re-introduced by upstream's changes. The per-patch agent MUST re-delete `web/` as part of the merge resolution + remove any new `pat/web` imports. Mitigation: post-merge sanity check `ls web/ 2>&1 | grep "No such"` AND `grep -rn "la5nta/pat/web" --include="*.go" .` (must return empty). This is intentional divergence per the [`project_fork_enables_aggressive_deletion`](../../../../.claude/projects/-home-administrator-Code-tuxlink/memory/project_fork_enables_aggressive_deletion.md) pattern; the per-patch overhead is bounded.
...
> **POST-PLAN-REVIEW SCOPE AMENDMENT (2026-05-18):** A subsequent 4-round plan-review cycle (R1 friction, R2 contract, R3 coverage, R4 Codex) surfaced 38 plan-review findings. During synthesis, the `project_fork_enables_aggressive_deletion` insight crystallized: **Pat's web UI is redundant in tuxlink's architecture** (tuxlink wraps Pat in Tauri; users never see Pat's web UI). The web-UI scope was changed from MODIFY to DELETE entirely (see §2.4 + §3.2 `web/` row + §3.1 architecture diagram). This SUPERSEDES the following adrev-disposition rows below (they targeted the modification path; the deletion path obviates them):
>
> - "Web UI form-POST silently drops secure_login_password after backend removal" (R3 P0 #2, R4 P2 #3) → MOOT: web UI doesn't exist post-delete
> - "Web UI/dist drift on upstream merge re-introducing the field" → SUPERSEDED by new §5.3 risk row "Upstream re-introduces `web/` directory at merge time"
>
> The adrev disposition table below documents the as-of-`68a698c` state; the plan's §8 disposition captures the additional plan-review-driven changes.

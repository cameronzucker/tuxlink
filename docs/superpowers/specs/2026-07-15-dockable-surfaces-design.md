...
Every popped surface leaves a visible trace at its original placement, and that trace is the way back: the menu reads "Routines ↗" and focuses the window; the map toggle reads "Map ↗" and focuses the window (label compacted from "Tac Map ↗ — in window" by AMD-3 below); the APRS dock tab renders a click-to-focus placeholder. A user who cannot remember where a window went can always follow the pathway from where the element used to live. No surface ever simply vanishes from the main window's vocabulary. Pathway affordances always carry their text label — never icon-only. This principle binds future dockable consumers, not just the first three.
...
- **Tac Map popped:** the reading pane returns to messages; the map expand toggle reads "Map ↗" (focuses, title text carries the full "in its own window" phrasing) with an adjacent ⇤ dock-back glyph (accessible-named "Dock the Tac Map back inline"). ⇤ foregrounds: it sets the APRS dock open and the map expanded (the inline placement's two preconditions), restoring the inline map. **AMD-3 (tuxlink-w68mb, operator design call 2026-07-20):** the dock header is a SINGLE row in every state — the mxqjp two-row surface bar was rejected as restyled jank. The pathway chip carries the slot's text label (the §1 rule); the dock-back compacts to the accessible-named glyph because it is the recovery action, not the pathway, and the popped window's own title bar carries the text-labeled "⇤ Dock back".
...
Pop-out entry points: an **↗ Pop out** affordance (text-labeled) in each surface's existing header — Routines dashboard AND designer header (popping from the designer carries the designer view in the token), the Tac Map's own top-right control cluster on the map surface (AMD-3: relocated from the dock header, whose single-row width budget the ~85px text button broke; pop-out of the surface you are looking at, matching every other entry point), the APRS chat panel header.

...
Steps:
- [x] Read the three decompile anchors above and confirm WLE's tactical-registration check is an **online** operation: every call to `AccountRegistered` / `TacticalAddressExists` in `Globals.cs` / `DialogAddAuxCallsign.cs` is fenced behind `objWL2KInterop.HaveInternetConnection(...)`, and the failure copy is "not registered with the Winlink system" (`DialogAddAuxCallsign.cs:511`). This rules out a purely-local check and rules in a server round-trip. Record: it is online-only, consistent with a Winlink-server lookup, NOT a CMS-telnet-protocol command issued during the B2F exchange. **Re-verified 2026-06-12 (cypress-raven-sandbar):** `Globals.cs:2253-2255` (`AddOneTacticalAddress`) branches `IsTacticalAddress` → `TacticalAddressExists` vs `AccountRegistered`; `DialogAddAuxCallsign.cs:495,511` confirms the "not registered with the Winlink system" refusal copy, and `:456` the `IsTacticalAddress` discriminator. Online-only confirmed.
...
- [x] **Write the confirmed mechanism into this plan file** (replace the "RECORDED FINDING" block below if your re-verification differs from it; otherwise check the box leaving it as-is). No executable task below proceeds on an unrecorded endpoint. **Endpoint CONFIRMED unchanged; prior-art grounding addendum added below.**
...
### PRIOR-ART GROUNDING ADDENDUM (cypress-raven-sandbar, 2026-06-12)

Re-verification per the operator's "ground against Pat/wl2k-go, don't guess" directive. Three sources cross-checked; the recorded endpoint **stands unchanged**, with the following grounding + one impl caveat:

1. **Live ServiceStack metadata (authoritative, fetched 2026-06-12 from `https://api.winlink.org/json/metadata?op=AccountTacticalExists`):** route is literally `POST,GET /account/tactical/exists`. Request DTO: `AccountTacticalExists{ TacticalAccount: string (body, required) }` + `WebServiceRequest{ Key: string (required) }`. Response DTO: `AccountTacticalExistsResponse{ Tactical: bool }` + `WebServiceResponse{ ResponseStatus }`. The documented POST sample body is byte-identical to the recorded finding: `{"TacticalAccount":"String","Key":"String"}` → `{"Tactical":false,"ResponseStatus":{…}}`. **The plan's POST + JSON-body shape is explicitly documented, not inferred.**

2. **Pat / wl2k-go prior art (`dev/scratch/tuxlink-pat/internal/cmsapi/{api,client}.go`, the canonical Go client):** confirms `RootURL = "https://api.winlink.org"`, and that the access **key is sent as the `?key=` URL query param on every request** (`client.go:59`, `newJSONRequest`), with existence checks issued as **GET** (`AccountExists` → `getJSON`). Pat does **NOT** implement a tactical-exists call at all — only `/account/exists` (the FULL-callsign companion op), `/account/add`, `/version/add`, `/account/password/validate`, `/gateway/status.json`. So prior art confirms the *base URL + auth mechanism + REST idiom* but does **not** independently confirm the tactical path — which rests on the WLE decompile method names + the live metadata above. Both POST(body-key) and GET(query-key) are valid per the `POST,GET` route; the plan's POST+body-key choice is sound. Pat ships a hardcoded WDT-issued key (`1880278F…`, "for use with Pat") — tuxlink must NOT reuse Pat's key; it plumbs its own via config (`cms_web_api_key`), fail-closing when absent.

3. **Impl caveat for Task 3 (carried into that task's prompt):** ServiceStack can return **HTTP 200 with a populated `ResponseStatus` error** (e.g. bad/expired key, rate-limited) and `Tactical:false`. Task 3's draft impl checks only `resp.status().is_success()` + parses `{Tactical}`, so a 200-with-error would cache `NotRegistered` (a definite negative) instead of staying `Unknown`. **Fail-closed safety still holds** (NotRegistered also refuses CMS), but to honor this finding's "remain `Unknown` on a ResponseStatus error" rule, the verifier MUST deserialize `ResponseStatus` and return `VerifyError::Http`/`Decode` when `ResponseStatus.ErrorCode` is non-empty, rather than mapping to a false definite-negative.

4. **Config field status:** `cms_web_api_key` does **not yet exist** in `src-tauri/src/config.rs` (grep-confirmed 2026-06-12). Per the plan's risk note #3, Phase 5 does not block on it — the verifier gets an empty key → `KeyMissing` → gate fail-closes (the safe default; tactical+CMS is refused until the key is configured). A follow-up bd issue tracks plumbing the config field (Phase 7 UI / config task).

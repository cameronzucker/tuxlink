...
1. **Does the BTECH "iMessage over radio" ride raw APRS UI frames over KISS, or the radio's native Benshi message path over GAIA/BLE?** HT Commander's source answers it. If native, Layer 2 implements the Benshi message path, not just APRS — changes the Phase 2 build. (Per the AI-amateur-radio-reliability rule: do not assume; read the reference implementation.)
   - **RESOLVED 2026-06-13 (vendor-app decompile — `com.benshikj.ht.btech.ham`, the official "HT" app; see `dev/scratch/benshi-re/DECOMPILE-FINDINGS.md`).** It is the **native Benshi message path**, not KISS. The vendor command enum (`v4/v.java`, ordinal = wire opcode) is a single connection multiplexing control + messaging + position: `HT_SEND_DATA`(31) TX, `RX_DATA`(57) + `DATA_RXD`(2) notification RX, `SET_MSG`/`GET_MSG`, `SET/GET_APRS_PATH`. The official app does control + chat over one link and **never mode-switches**. Cross-verified against benlink `BasicCommand` (exact match 0–76). **Consequence — Phase 2 unified native model:** on a UV-Pro, carry APRS over native GAIA so control + chat share one connection (control strip **always-live**, no handoff/confirm, no self-inflicted `LinkBusy`); KISS floor stays for generic Classic-SPP TNCs. Backend slice: **tuxlink-7my9** (native `HT_SEND_DATA` TX + `RX_DATA`/`DATA_RXD` RX), extending the control-only nx95 backend. Frontend: **tuxlink-ve3j** (always-live control surface). The earlier "KISS xor native control, single-host mode-switch" framing was an artifact of two separate RFCOMM backends, **not** a radio limitation.
...
3. UV-Pro KISS-vs-native split: what does the radio expose over plain KISS vs only over the native protocol? (Decides what Layer 1 alone can do on the UV-Pro.)
   - **RESOLVED 2026-06-13 (same decompile).** "KISS" in the vendor app is a **radio settings toggle** (`kiss_en` switch / `toggle_kiss` string) that exposes the UV-Pro as a dumb external KISS TNC over SPP/RFCOMM — **Layer 1's path**. Everything the app itself does (control, channel/freq/mode, messaging, position, APRS path) is the **native GAIA** protocol — **Layer 2's path**. So Layer 1 on the UV-Pro can do messaging only (radio-as-dumb-TNC, KISS enabled); Layer 2 (native) does messaging **and** control over one connection.

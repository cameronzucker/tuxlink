...
- [ ] **🔥 MAP-PERF-1 — A perf forecast from the mocked render-harness is not an app-level prediction.** The front-end render-harness (`dev/render-harness/`) uses canned Tauri data and a trivial scene (no real tile decode, no markers, no pack compositing); its fps number MUST NOT gate a perf-sensitive map ship. Map perf claims require an on-Pi frame-timing measurement of the **real map component** (the live `src/map/LeafletMap.tsx` scene) with a region pack + station pins + the Maidenhead grid mounted at real resolution under software GL (llvmpipe), measuring p50/p95 frame time during a scripted pan/zoom. (Found in tuxlink-vnk7, 2026-06-15: the ~45 fps "forecast" was measured on the front-end-only render-harness, so it never predicted real app perf — software GL on the Pi makes the real tile-decode + composite a different fill-rate problem.) **No such harness currently exists:** the original `dev/perf-harness/` was MapLibre-specific and was retired with the Leaflet basemap migration (tuxlink-6ewsw); build a Leaflet-scene equivalent fresh if a perf-sensitive map change needs gating.

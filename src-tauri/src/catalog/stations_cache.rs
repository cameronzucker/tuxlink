...
impl StationsCache {
        ...
        F: Future<Output = Result<StationListing, E>>,
    {
        ...
        match fetch.await {
            ...
            Err(e) => {
                // Stale-on-error: serve the last good entry (with its original timestamp) if any.
...
mod tests {
    ...
    async fn concurrent_same_key_callers_coalesce_to_one_fetch() {
        let clock = Arc::new(MockClock::new(0));
        let cache = Arc::new(StationsCache::new(60_000, clock));
        let calls = Arc::new(AtomicUsize::new(0));
        let slow_fetch = || {
            let c = calls.clone();
            async move {
    ...
    async fn error_with_empty_cache_propagates_err() {
        let clock = Arc::new(MockClock::new(0));
        let cache = StationsCache::new(60_000, clock);
        let res = cache
            .get_or_fetch(key(ListingMode::VaraHf), async { Err::<StationListing, String>("network down".into()) })
            .await;

...
/// Measure RMS over ~150 ms of real frames (7200 @ 48 kHz mono).
const MEASURE_FRAMES: usize = 7_200;
...
/// The read + RMS loop (spec §NewCommands: "wait ≥1 period → RMS over ~150 ms").
/// Discards the first full period, then accumulates ~150 ms of real frames and
/// returns the level. Returns `Err` only if the source itself errors — the
/// caller maps that to an `in-use`/`error` `MeterDto`. A source that opens but
/// delivers nothing but empty batches yields a finite floored value, never
/// `NaN` (the "single nonblocking read gives NaN" hazard the tests pin).
pub(crate) fn meter_read<S: SampleSource>(src: &mut S) -> Result<MeterDto, SourceError> {
    ...
    // Phase 2: accumulate sum-of-squares over ~150 ms of delivered frames.
    let mut sum_sq = 0f64;
    let mut n = 0usize;
    ...
    Ok(MeterDto { rms_dbfs, state: state.into() })
}

/// Open the device, run [`meter_read`], close. CI-compile-checked only (needs
/// real ALSA). Maps a busy device to `in-use` and any other open/read failure
/// to `error`; a clean read returns `live`/`silent`. The caller MUST already
/// hold the meter reservation (`DeviceReservation::try_meter`) across this call.
pub(crate) fn open_and_meter(alsa_hw: &str) -> MeterDto {
    match crate::ft8::alsa_source::AlsaSource::open(alsa_hw) {
        Ok(mut src) => match meter_read(&mut src) {
            Ok(dto) => dto,
...
mod tests {
    ...
    fn meter_read_on_silent_source_is_silent_and_finite() {
        // 1 discard period + 2 measure periods of pure-zero frames.
        ...
        let dto = meter_read(&mut src).unwrap();
    ...
    fn meter_read_propagates_source_error() {
        ...
        assert_eq!(meter_read(&mut src), Err(SourceError::Busy));
    }

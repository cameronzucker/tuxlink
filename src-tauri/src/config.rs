...
pub struct ArdopUiConfig {
    ...
    /// chain (verified on-air 2026-06-25). The sound-card playback gain sets
    /// actual RF power; this caps the digital peak so the modulation stays clean.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// tune-aware, so the call must sustain ConReqs until the gateway is ready;
    /// the prior fixed value of 3 (~10 s) was too short to raise one
    /// (2026-06-25). Clamped to 2..=30 when read.
    #[serde(default, skip_serializing_if = "Option::is_none")]

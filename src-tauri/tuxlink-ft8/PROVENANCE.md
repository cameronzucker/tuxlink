...
| Fine refinement (T2.2) | maximize Costas cross-energy over time ±40 ms (2 ms steps) and frequency ±two tones (±12.5 Hz, 0.25 Hz steps). Frequency span widened from WB2FKO's ±2.5 Hz to ±2 tones because the dB-contrast coarse metric can mislocate an off-grid carrier by >1 tone (measured ~6.6 Hz on gen_ft8 off-grid carriers), while the cross-energy objective is sharply unimodal at the true carrier. (`sync.rs` `fine_refine`) | WB2FKO fine sync (`ft8b`/`sync8d`: ±40 ms time, sub-Hz `∆f`); span adaptation documented in `fine_refine` |

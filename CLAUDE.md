...
- **Model ceiling — GPT-5.5 ([ADR 0023](docs/adr/0023-ban-gpt-5-6-until-deception-assessed.md) clauses 1-4; shadow program ended by [ADR 0026](docs/adr/0026-end-gpt56-shadow-assessment-retain-ban.md)):** every adrev round runs on GPT-5.5 (the default Codex provider/auth); it alone satisfies the "at least one round via Codex" requirement. GPT-5.6 remains banned for ALL Tuxlink tasks pending further testing — the 2026-07-19/20 shadow-assessment program is COMPLETE (16 pairs, `dev/gpt56-assessment-ledger.md` frozen as the evidence record); do not run shadow rounds, and do not treat their absence as a process defect. The ADRs are canonical; this bullet is the pointer.

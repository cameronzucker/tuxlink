...
- **Model ceiling — GPT-5.5 authoritative, GPT-5.6 SHADOW only ([ADR 0023](docs/adr/0023-ban-gpt-5-6-until-deception-assessed.md), amended 2026-07-19):** the authoritative round runs on GPT-5.5 (unchanged: it alone satisfies the "at least one round via Codex" requirement; a round that ran on 5.6 does not, and 5.6 stays banned for every non-shadow task). Additionally, per the ADR's clause-5 assessment protocol, EVERY adrev now runs a second, NON-authoritative shadow round on GPT-5.6 over the same diff and prompt: repeat the same invocation via OpenRouter (the ChatGPT-account auth cannot reach 5.6): `OPENROUTER_API_KEY="$(secret-tool lookup service elmer-openrouter account teacher)" npx --yes @openai/codex review -c model_provider=openrouter -c model="openai/gpt-5.6-sol" -` (provider block lives in `~/.codex/config.toml`, `wire_api = "responses"` — Codex 0.140+ rejects `"chat"`; Sol is the flagship coding tier, the fair Codex-default analogue; key stays in the OS keyring, env-at-invocation only). Instruct the shadow prompt to verify by grep/read only — 5.6-sol otherwise attempts cargo builds on this contended Pi. Tee to `dev/adversarial/<date>-<topic>-codex-gpt56.md`, run BOTH rounds against the SAME commit, ground any 5.6-only finding against source before acting on it, and add a paired entry to [`dev/gpt56-assessment-ledger.md`](dev/gpt56-assessment-ledger.md) (quality delta + deception/cheating indicators — the ledger carries the rubric). The ADR is canonical; this bullet is the recipe.

...
6. Write a session-end handoff document to `dev/handoffs/<YYYY-MM-DD>-<short-slug>.md` as a **thin scaffolding pointer per [ADR 0031](docs/adr/0031-handoffs-are-thin-pointers-to-full-fidelity-anchors.md)**: one framing paragraph, pointers to the full-fidelity anchor for each in-flight line (spec/plan, ADR, bd issue/epic), open PR numbers, the critical first action or gate, and the worktree enumeration per [ADR 0009](docs/adr/0009-worktree-disposal-ritual.md). Handoffs never carry ledgers, findings, or decisions that lack a full-fidelity home — anchor first, then point. Cross-session state = anchors + bd + PRs + git history + a quick repo grep, never reconstruction from handoff prose.

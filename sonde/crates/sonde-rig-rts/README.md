...
sonde hardware bring-up (tuxlink-mxyz / tuxlink-9ggl); eventually
rolls into the unified `sonde-rig` crate per ADR 0015.
...
This is **NOT** the same mechanism as `sonde-rig-cm108` (which uses the
CM108-family USB-HID GPIO mechanism for DRA-100 / SignaLink / AIOC
class adapters). Both are valid PTT backends; the future `sonde-rig`
...
```rust
use sonde_rig_rts::{LinuxTty, Ptt, RtsPtt};
...
The `sonde-rig-rts` binary is the operator's bench-validation tool.
...
sonde-rig-rts release --device /dev/digirig
```
...
AGPL-3.0-only, matching the rest of the `sonde/` workspace.

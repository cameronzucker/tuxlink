//! `Ptt` trait + state.
//!
//! Duplicated from `sonde-rig-cm108`'s `ptt` module so each backend
//! crate stands alone for now. The future `sonde-rig` umbrella crate
//! (per ADR 0015 and the locked `tuxlink-5jb` decision) will lift
//! these into a `sonde-rig-core` and re-export from both backend

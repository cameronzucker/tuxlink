...
Phase 1 of the sonde hardware bring-up (tuxlink-u1js / tuxlink-9ggl);
eventually rolls into the unified `sonde-rig` crate per ADR 0015.
...
```rust
use sonde_rig_cm108::{Cm108Ptt, GpioPin, HidrawWriter, Ptt};
...
The `sonde-rig-cm108` binary is the operator's bench-validation tool.
...
sonde-rig-cm108 release --device /dev/dra100-ptt
```

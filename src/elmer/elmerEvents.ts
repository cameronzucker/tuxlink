...
/**
 * Context-usage snapshot from a completed turn (T7 / EV_CONTEXT). Emitted by
 * either the native Ollama `/api/chat` adapter or the OpenAI-compat adapter
 ...
 *                   compat `usage.prompt_tokens`).
 *   evalTokens    — tokens generated in the completion (Ollama `eval_count` /
 *                   compat `usage.completion_tokens`).
 *   numCtx        — context window: always known on the native Ollama path
 *                   (`options.num_ctx`); on the compat path, the probed
 *                   `/v1/models` window when advertised, else `null` when the
 *                   window is unknown — the meter then renders a bare token
 *                   counter with no percentage (counter-mode).
 */
export interface ElmerContextPayload {
  kind: 'context';
  /** Tokens consumed by the prompt (Ollama `prompt_eval_count` / compat `usage.prompt_tokens`). */
  promptTokens: number;
  /** Tokens generated by the model (Ollama `eval_count` / compat `usage.completion_tokens`). */
  evalTokens: number;
  /** Context window (native Ollama num_ctx, always known; compat probe, `null` when unknown → counter-mode). */
  numCtx: number | null;
}

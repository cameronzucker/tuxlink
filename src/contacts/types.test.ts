// Value-level pins for every contacts-reachability TS union's exact
// kebab-case wire string. BINDING AMENDMENT (Task 22 / CLD-7, carried over by
// Task T-E): a union member with the WRONG value (e.g. `'observed_incoming'`
// instead of `'observed-incoming'`) typechecks fine — TS has no idea what the
// "correct" string is — but breaks at runtime the instant the backend emits
// the real wire string. Only a value-level test (not a type-level one) catches
// that class of bug.
//
// Source of truth for every string below: `src-tauri/src/contacts/reachability.rs`,
// all `#[serde(rename_all = "kebab-case")]`. The Rust shape tests
// (`tier_wire_shape_is_pinned_exactly`, `enum_wire_tags_are_kebab_case`,
// `bandwidth_wire_shape_is_pinned_exactly`) pin the SAME strings on the Rust
// side; this file is the TS-side mirror — the direct successor of the deleted
// `peers/types.test.ts` (the peer model these pins used to guard is gone; the
// reachability enums moved verbatim into contacts).
//
...
// below each one is the value-level pin a reviewer diffs against reachability.rs.
...
describe('contacts reachability union wire-value pins (reachability.rs kebab-case source of truth)', () => {
  it('ContactTier', () => {
    const exhaustive: Record<ContactTier, true> = {
      confirmed: true,
      unconfirmed: true,
      unknown: true,
    };
    expect(Object.keys(exhaustive).sort()).toEqual(
      ['confirmed', 'unconfirmed', 'unknown'].sort(),
    );
  });
  ...
  it('GridSource — the pivot DROPPED the peer-model "contact" variant', () => {
  ...
  it('ChannelBandwidth — internally tagged on "kind", each kind value pinned', () => {
    ...
    // Mirrors reachability.rs's `bandwidth_wire_shape_is_pinned_exactly`: the
    // exact JSON the backend emits for the data-carrying variant is

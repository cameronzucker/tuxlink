...
This is a **discipline gate**, not advice. If a primary flow is broken, the feature is not shipped — it is partially-wired, which is a defect.
...
If the flows were captured at feature start as the definition-of-done, use those verbatim instead of re-asking.
...
Trace the flow **verbatim as the operator stated it.** You may not narrow, merge, reinterpret, or substitute a flow to make it pass (see Red Flags).
...
|---|---|---|
| **Orphan producer** | Backend fn/command with zero callers | grep its name from the consumer side → empty |
...
| **Unsatisfiable gate** | A precondition (`is_connected`, `active_identity`, `has_secret`) no path satisfies | trace what sets the precondition; is anything calling it? |
...
- ⚠️ **wired but unverifiable here** — e.g., on-air RF, live external service. Wiring is proven; the terminal effect can't be checked in this environment. Say so explicitly; do not upgrade it to ✅.

### 5. Gate

If **any** operator flow is ❌, the feature is **NOT shipped**. Do not claim done, do not mark the PR ready, do not close the issue. The remaining wiring is the real work, not a follow-up. Report the broken flows with their break points; that list is the spec for finishing the feature.
